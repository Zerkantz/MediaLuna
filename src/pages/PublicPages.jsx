...
const getAvailableDatesForSalon = (salon, availability) => {
  if (availability.length) return availability.filter((item) => item.estado === 'disponible').map((item) => item.fecha).sort()
  return salon?.availableDates ?? []
}
...
export function BookingDatePage() {
  ...
  const availableDates = getAvailableDatesForSalon(salon, salonAvailability)
