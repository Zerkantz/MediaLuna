...
const toFirestoreData = (collectionName, input, isCreate = false) => {
  ...
  if (collectionName === COLLECTIONS.salones) {
    ...
    if (isCreate || data.availableDates !== undefined) data.availableDates = asArray(data.availableDates).map(normalizeDateOnly)
    if (isCreate || data.photos !== undefined) data.photos = asArray(data.photos)
    if (isCreate || data.serviciosIds !== undefined) data.serviciosIds = asArray(data.serviciosIds)
  ...
  if (collectionName === COLLECTIONS.disponibilidad) {
    if (isCreate || data.salonesIds !== undefined) data.salonesIds = asArray(data.salonesIds)
    if (data.fecha !== undefined) data.fecha = normalizeDateOnly(data.fecha)
  }

  if (collectionName === COLLECTIONS.reservaciones) {
    if (isCreate || data.salonesIds !== undefined) data.salonesIds = asArray(data.salonesIds)
    if (isCreate || data.serviciosIds !== undefined) data.serviciosIds = asArray(data.serviciosIds)
    if (isCreate || data.duenoId !== undefined) data.duenoId = Array.isArray(data.duenoId) ? data.duenoId[0] ?? '' : data.duenoId ?? ''
    if (isCreate || typeof data.fechaCreacion === 'string') data.fechaCreacion = serverTimestamp()
