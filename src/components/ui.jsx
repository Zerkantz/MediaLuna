...
export function StatusBadge({ status }) {
  const tone = ['pagado', 'confirmada', 'disponible', 'activo'].includes(status) ? 'success'
    : ['pendiente', 'por confirmar', 'reservada', 'reservado', 'reembolso_pendiente'].includes(status) ? 'warning'
      : ['bloqueada', 'bloqueado', 'cancelada', 'inactivo', 'fallido', 'expirado', 'reembolsado', 'reembolso_parcial', 'reembolso_fallido', 'disputado'].includes(status) ? 'danger' : 'neutral'
